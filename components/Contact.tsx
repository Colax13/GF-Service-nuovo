...
import { ArrowRight, Check, ChevronDown, Calendar, MapPin, Building, Phone, ArrowLeft, X, Mail, User, MessageSquare, Users, HelpCircle, Briefcase } from 'lucide-react';
...
const Contact: React.FC<ContactProps> = ({ simpleMode = false }) => {
  ...
  // Automatically scroll to the top of the section when switching to the advanced form
  useEffect(() => {
  ...
  return (
    <section id="contatti" ref={sectionRef} className="relative pt-32 pb-24 min-h-screen flex flex-col items-center justify-start overflow-hidden bg-gf-darker">
       ...
       {/* Background Image: Dynamic blur based on state */}
       <div 
       ...
       <div className="relative z-10 container mx-auto px-6 w-full max-w-6xl">
          ...
          {/* Header Section: Hero-like Page Title */}
          ...
          <div className="relative w-full min-h-[500px]">
            ...
            >
                ...
                <div className="w-full bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 mb-16 shadow-2xl">
                    <form className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                        {/* Row 1: Personal Info (3 items on LG) */}
                        <div className="relative group md:col-span-1 lg:col-span-2">
                            <User className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                            <input type="text" placeholder="Nome" className="w-full h-14 pl-12 pr-4 bg-black/20 text-white placeholder:text-white/40 rounded-xl outline-none focus:bg-black/40 focus:ring-2 focus:ring-gf-green/50 transition-all" />
                        </div>
                        <div className="relative group md:col-span-1 lg:col-span-2">
                            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                            <input type="email" placeholder="Email" className="w-full h-14 pl-12 pr-4 bg-black/20 text-white placeholder:text-white/40 rounded-xl outline-none focus:bg-black/40 focus:ring-2 focus:ring-gf-green/50 transition-all" />
                        </div>
                        <div className="relative group md:col-span-1 lg:col-span-2">
                            <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                            <input type="tel" placeholder="Telefono" className="w-full h-14 pl-12 pr-4 bg-black/20 text-white placeholder:text-white/40 rounded-xl outline-none focus:bg-black/40 focus:ring-2 focus:ring-gf-green/50 transition-all" />
                        </div>

                        {/* Row 2: Event Type (Full width on LG to separate) */}
                        <div className="relative group md:col-span-1 lg:col-span-6">
                            <select className="w-full h-14 px-4 bg-black/20 text-white/80 rounded-xl outline-none focus:bg-black/40 focus:ring-2 focus:ring-gf-green/50 transition-all appearance-none cursor-pointer">
                        ...
                        <button type="button" className="col-span-1 md:col-span-2 lg:col-span-6 h-14 bg-gf-green hover:bg-emerald-600 text-white font-bold uppercase tracking-widest text-sm rounded-xl shadow-lg hover:shadow-emerald-900/40 hover:-translate-y-1 transition-all flex items-center justify-center gap-2">
                    ...
                    <div className="text-center mt-4">
                         <label className="text-xs text-white/40 flex items-center justify-center gap-2 cursor-pointer hover:text-white/60 transition-colors">
                            <input type="checkbox" className="rounded bg-white/10 border-white/20 text-gf-green focus:ring-0" />
                            Accetto la privacy policy e il trattamento dei dati personali.
                         </label>
                    </div>
                </div>

                {/* 2. "Maggiori Informazioni" Section Trigger - HIDDEN IN SIMPLE MODE */}
                {!simpleMode && (
                    <div className="w-full max-w-4xl text-center relative py-8">
                        ...
                        <h3 className="text-2xl md:text-3xl text-white font-bold mb-4 mt-8">
                            Hai un progetto complesso?
                        </h3>
                        <p className="text-gray-400 mb-8 max-w-xl mx-auto font-light">
                            Se conosci già i dettagli tecnici, le date e la location, aiutaci a servirti meglio compilando la scheda completa.
                        </p>

                        <button 
                            onClick={() => setIsAdvanced(true)}
                            className="group relative inline-flex items-center justify-center gap-3 px-10 py-5 bg-transparent border border-white/20 hover:border-gf-green/50 hover:bg-white/5 rounded-full text-white transition-all duration-300 overflow-hidden"
                        >
            ...
            {/* Only rendered if NOT in simple mode (or if state allows, but simple mode hides trigger) */}
            <div 
            ...
            >
                <div className="bg-gf-darker/80 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-12 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] relative overflow-hidden">
                    ...
                    <button 
                        ...
                        className="absolute top-6 right-6 p-2 text-white/30 hover:text-white hover:bg-white/10 rounded-full transition-all z-20 group"
                        title="Chiudi"
                    >
                        <X size={24} className="group-hover:rotate-90 transition-transform duration-300" />
                    </button>

                    <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 relative z-10">
                        {/* Form Content Omitted for brevity since logic handles display */}
                         {/* --------------------------------------------------------
                             SEZIONE 1: Nome, Cognome, Telefono, Email
                        --------------------------------------------------------- */}
                        ...
                        <div className="space-y-2">
                            ...
                            <input type="text" className="w-full h-12 px-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all" />
                        </div>
                        <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input type="tel" className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all" />
                            </div>
                        </div>
                         <div className="space-y-2">
                            <label className="text-xs text-gf-green font-bold uppercase tracking-wider ml-1">Email *</label>
                            <input type="email" className="w-full h-12 px-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all" />
                        </div>


                        {/* --------------------------------------------------------
                             SEZIONE 2: Profilo & Background (Merged)
                        --------------------------------------------------------- */}
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <select className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all appearance-none cursor-pointer">
                         ...
                         <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all" />
                            </div>
                        </div>

                         <div className="space-y-2">
                            <label className="text-xs text-gray-400 font-bold uppercase tracking-wider ml-1">Hai già organizzato eventi?</label>
                            <div className="relative">
                                <select className="w-full h-12 px-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all appearance-none cursor-pointer">
                                    <option className="bg-gf-darker">Seleziona...</option>
                                    <option className="bg-gf-darker">Sì</option>
                                    <option className="bg-gf-darker">No, è il primo</option>
                                </select>
                                <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs text-gray-400 font-bold uppercase tracking-wider ml-1">Come ci hai conosciuti?</label>
                            <div className="relative">
                                <HelpCircle size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
                                <select className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all appearance-none cursor-pointer">
                                    <option className="bg-gf-darker">Seleziona...</option>
                                    <option className="bg-gf-darker">Sito web</option>
                                    <option className="bg-gf-darker">Social Media</option>
                                    <option className="bg-gf-darker">Cartellonistica</option>
                                    <option className="bg-gf-darker">Passaparola</option>
                                    <option className="bg-gf-darker">Altro</option>
                                </select>
                                <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none" />
                            </div>
                        </div>
                        ...
                        {/* --------------------------------------------------------
                             SEZIONE 3: Dove, Quando, Partecipanti, Note (Renumbered)
                        --------------------------------------------------------- */}
                        <div className="md:col-span-2 mb-2 mt-4 flex items-center gap-4">
                         ...
                         <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" placeholder="Città / Location" className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" placeholder="Data precisa o indicativa" className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all" />
                            </div>
                        </div>

                         <div className="md:col-span-2 space-y-2">
                            <label className="text-xs text-gray-400 font-bold uppercase tracking-wider ml-1">Numero Partecipanti Previsti</label>
                            <div className="relative">
                                <Users size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
                                <select className="w-full h-12 pl-10 pr-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all appearance-none cursor-pointer">
                                    <option className="bg-gf-darker">Seleziona...</option>
                                    <option className="bg-gf-darker">Meno di 50</option>
                                    <option className="bg-gf-darker">Da 50 a 100</option>
                                    <option className="bg-gf-darker">Da 100 a 300</option>
                                    <option className="bg-gf-darker">Da 300 a 500</option>
                                    <option className="bg-gf-darker">Più di 500</option>
                                </select>
                                <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none" />
                            </div>
                        </div>

                         {/* Services Selection Grid (New Feature) */}
                                ...
                                {['Tensostrutture', 'Palchi e Pedane', 'Sedie e Tavoli', 'Gazebo', 'Casette in Legno', 'Impianti Audio/Luci', 'Spillatori', 'Bagni Chimici', 'Generatore', 'Transenne', 'Riscaldamento', 'Pavimentazione'].map((item) => (
                                    <label key={item} className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-lg cursor-pointer hover:bg-white/10 hover:border-gf-green/50 transition-all group">
                                        <div className="relative flex items-center">
                                          <input type="checkbox" className="peer appearance-none w-5 h-5 rounded border border-white/30 checked:bg-gf-green checked:border-gf-green transition-all" />
                        ...
                        <div className="md:col-span-2 space-y-2">
                            ...
                            <textarea rows={4} className="w-full p-4 bg-white/5 border border-white/10 rounded-lg text-white focus:border-gf-green focus:bg-white/10 outline-none transition-all resize-none placeholder:text-white/20" placeholder="Descrivi le strutture, le dimensioni o altre necessità particolari..."></textarea>
                        </div>
                        ...
                        <div className="md:col-span-2 pt-8 flex flex-col-reverse md:flex-row items-center justify-between gap-6 border-t border-white/10 mt-4">
                            <button 
                                type="button" 
                                onClick={() => setIsAdvanced(false)}
                                className="text-gray-400 hover:text-white text-sm font-medium flex items-center gap-2 transition-colors group"
                            >
                                <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> 
                                Torna alla richiesta rapida
                            </button>
                            
                            <button type="submit" className="w-full md:w-auto px-10 py-4 bg-gf-green hover:bg-emerald-600 text-white font-bold uppercase tracking-widest text-sm rounded-full shadow-[0_10px_20px_rgba(0,112,90,0.3)] hover:shadow-[0_15px_30px_rgba(0,112,90,0.5)] transform hover:-translate-y-1 transition-all">
